...
export default function FlashcardCard({
  ...
  const getDaysUntilReview = () => {
    ...
    const days = Math.ceil((new Date(nextReviewDate) - new Date()) / (1000 * 60 * 60 * 24));
    return days;
  };
  ...
  const formatReviewInfo = () => {
    if (status !== 'Done' || !nextReviewDate) {
      return `${hintsUsed}/${hintCount} hints${attempts > 0 ? ` • ${attempts} attempts` : ''}`;
    }
    ...
    if (daysUntilReview <= 30) return `📅 ${Math.ceil(daysUntilReview / 7)} weeks`;
    return `📅 ${Math.ceil(daysUntilReview / 30)} months`;
  };

  const getStatusStyling = () => {
    if (status === 'Done' && daysUntilReview !== null) {
      if (daysUntilReview <= 0) return 'text-red-400 font-semibold animate-pulse';
  ...
  const getDifficultyIndicator = () => {
    ...
    return (
      <span className={`text-xs ${colorClass}`} title={`Difficulty: ${difficultyFactor.toFixed(1)}`}>
        {indicator}
      </span>
    );
  };
    ...
    <Link to={`/trainer/${_id}`} className="block">
      <div
        className={clsx(
          ...
          color,
          status === 'Done' && daysUntilReview !== null && daysUntilReview <= 0 && "ring-2 ring-red-400/60 shadow-red-400/20"
        )}
      >
